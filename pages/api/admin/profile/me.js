...
export default withSessionRoute(async function (req, res) {
    ...
    if (req.method == "GET") {
        try {
            try {
                const user = await User.findOne({ _id: req.session.admin.id }).select('-_id -password -createdOn -updatedOn -status -type')
        ...
        try {
            try {
                ...
                const user = await User.findByIdAndUpdate({ _id: req.session.admin.id }, {
